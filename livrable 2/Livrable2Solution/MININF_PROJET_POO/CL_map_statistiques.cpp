...
SELECT
    ...
    ST.sto_quantite AS quantite_stock,
    P.pri_prix AS prix_achat_unitaire,
    ST.sto_quantite * P.pri_prix AS valeur_achat_stock
FROM
    STOCK S
JOIN
...
WHERE
    ST.sto_quantite < sto_seuilReapprovis;
...
System::String^ NS_Comp_MappageStatistiques::statistiques::totalValeurAchatStock(void)
{
    return "SELECT "
        ...
        "P.pri_prix AS prix_achat_unitaire, "
        "ST.sto_quantite * P.pri_prix AS valeur_achat_stock, "
        "SUM(ST.sto_quantite * P.pri_prix) OVER (PARTITION BY S.id_stock) AS somme_valeur_achat_stock "
        "FROM "
        "STOCK S "
...
System::String^ NS_Comp_MappageStatistiques::statistiques::totalValeurCommercialeStock(void)
{
    return
        ...
        "   ST.sto_quantite * P.pri_prix AS valeur_achat_stock,"
        "   SUM(ST.sto_quantite * P.pri_prix) OVER(PARTITION BY S.id_stock) AS somme_valeur_achat_stock,"
        "   SUM((SUM(ST.sto_quantite * P.pri_prix) * @marge * @tva) - (SUM(ST.sto_quantite * P.pri_prix) * @remise) - (SUM(ST.sto_quantite * P.pri_prix) * @demarqueInconnue)) OVER(PARTITION BY S.id_stock) AS totale_Valeur_commerciale"
        " FROM"
        "   STOCK S"
        "   JOIN"
        "   stocker ST ON S.id_stock = ST.id_stock"
        ...
        " GROUP BY"
        "   S.id_stock, A.art_designation, ST.sto_quantite, P.pri_prix;";
